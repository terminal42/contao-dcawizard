...
var dcaWizard = new Class({
	...
	initialize: function(element, options)
	{
		...
		this.request = new Request.DCAWizard(
		{
			...
			onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript)
			{
				...
				{
					this.request.send({url:responseHTML, method:'get'});
					return;
				}
	...
	sendOperation: function(event)
	{
		...
		// Hack for toggleVisibility
		if(button.dcaclick && button.dcaclick.toString().contains('AjaxRequest.toggleVisibility'))
		{
			return this.toggleVisibility(button);
		}
	...
	toggleVisibility: function(el) {
		...
		var url = new URI(el.get('href'));
		var id = url.getData('id');
		var table = url.getData('table');
		...
		if (publish) {
			...
			new Request({'url':url}).get({'state':1});
		} else {
			image.src = image.src.replace('visible.gif', 'invisible.gif');
			new Request({'url':url}).get({'state':0});
		}
