...
class DcaWizard extends Widget
{
    ...
    public function validate(): void
    {
        if ($this->mandatory) {
            ...
            if (!$objRecords->numRows && empty($this->strLabel)) {
    ...
    public function generate()
    {
        ...
        $objTemplate->dcaLabel = fn ($field) => System::getContainer()->get(Formatter::class)->dcaLabel($this->foreignTable, $field);

        $objTemplate->dcaValue = fn ($field, $value) => System::getContainer()->get(Formatter::class)->dcaValue($this->foreignTable, $field, $value, $this->dataContainer);

        $objTemplate->generateGlobalOperation = fn ($operation) => $this->generateGlobalOperation($operation);
        ...
        if (null === $varCallback) {
            ...
            $objTemplate->generateOperation = fn ($operation, $row) => $this->generateRowOperation($operation, $row);
        } else {
            $strCallback = '';
    ...
    public function generateRowOperation($operation, $row)
    {
        ...
        $attributes = !empty($def['attributes']) ? ' '.ltrim(sprintf($def['attributes'], $id, $id)) : '';
